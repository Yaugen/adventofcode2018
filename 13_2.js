...
const tick = (field, carts) => {
  ...
  for(let i = 0; i < carts.length; i++ ) {
    ...
    if(collision) {
      collisions.push(cart, collision);
    }
  }
  return carts.filter(cart => !collisions.includes(cart));
}
...
const generate = (field, steps, verbose = false) => {
  ...
  let carts = getCartsAndRestoreInput(rawInput);
  while(carts.length > 1) {
    carts = tick(field, carts);
